<div id="show-tekMessage" class="content scaffold-show" role="main">
	<ol class="property-list tekMessage">
				...
				<g:if test="${tekMessageInstance?.evento}">
				<li class="fieldcontain">
					<span id="evento-label" class="property-label"><g:message code="tekMessage.evento.label" default="Evento" /></span>
					
						<span class="property-value" aria-labelledby="evento-label"><g:link controller="tekEvent" action="show" id="${tekMessageInstance?.evento?.id}">${tekMessageInstance?.evento?.encodeAsHTML()}</g:link></span>
	...
	<g:form>
		...
		<g:link class="create" action="reply" resource="${tekMessageInstance}">
			Reply
		</g:link>
		</fieldset>
	</g:form>
